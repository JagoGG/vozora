 ...
 */
async startVozoraKeysRecording(bindingId: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("start_vozora_keys_recording", { bindingId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async stopVozoraKeysRecording() : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("stop_vozora_keys_recording") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async confirmPendingPaste() : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("confirm_pending_paste") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async cancelPendingPaste() : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("cancel_pending_paste") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async changeDictationModeSetting(mode: DictationMode) : Promise<null> {
    return await TAURI_INVOKE("change_dictation_mode_setting", { mode });
},
async changeAppProfilesSetting(profiles: AppProfile[]) : Promise<null> {
    return await TAURI_INVOKE("change_app_profiles_setting", { profiles });
},
 ...
 */
window_title_patterns: string[]; dictation_mode: DictationMode; paste_method?: PasteMethod | null }
 ...
 * - `natural_corrected`: run the existing LLM post-processing pipeline
 *   (grammar/filler-word cleanup) but nothing coding-specific.
 * - `ai_prompt`: treat the transcription as a prompt to send verbatim —
 *   intended for dictating into a chat/prompt box such as Claude Code's.
 * - `code`: run the coding-mode phrase table so spoken punctuation
 *   ("open paren", "abre llave", ...) becomes literal characters.
 * - `terminal_command`: run coding-mode AND, if the resulting text looks
 *   destructive, require an explicit user confirmation before pasting.
 */
export type DictationMode = "literal" | "natural_corrected" | "ai_prompt" | "code" | "terminal_command"
...
export type KeyboardImplementation = "tauri" | "vozora_keys"
