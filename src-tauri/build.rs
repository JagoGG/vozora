fn main() {
    ...
    // $ORIGIN-relative rpath into the `vozora` binary so it finds libtranscribe
    // next to it in the package — AppImage `usr/bin/vozora` -> `usr/lib`, and
    // deb/rpm `/usr/bin/vozora` -> `/usr/lib`. transcribe's
    ...
    // next to vozora.exe so the app loads our baseline build instead of statically
...
///
/// Vozora's native stack links the VC++ runtime dynamically (/MD). Shipping the
/// DLLs beside `vozora.exe` covers machines with no redistributable installed and
/// machines whose system redist is older than the CI toolset (issue #1527).
///
/// Driven by `VOZORA_VC_REDIST_DIRS`, set by CI to the redist dirs from the same
/// Visual Studio install that compiled the native code. Copies only the runtime
/// DLL families Vozora imports and no-ops when the env var is unset.
fn stage_vc_runtime_dlls() {
    use std::path::PathBuf;

    println!("cargo:rerun-if-env-changed=VOZORA_VC_REDIST_DIRS");

    let Some(redist_dirs) = std::env::var_os("VOZORA_VC_REDIST_DIRS") else {
        return;
    };
    ...
    for dir in std::env::split_paths(&redist_dirs) {
        for entry in std::fs::read_dir(&dir)
            .unwrap_or_else(|e| panic!("VOZORA_VC_REDIST_DIRS: read {}: {e}", dir.display()))
            .flatten()
        {
            let src = entry.path();
            let name = src
    ...
    for required in ["msvcp140.dll", "vcruntime140.dll"] {
        if !copied.iter().any(|n| n == required) {
            panic!(
                "VOZORA_VC_REDIST_DIRS is set but {required} was not found in it; \
                 the app-local VC++ runtime would be incomplete and Vozora would \
...
/// `vozora.exe` (Windows resolves DLLs from the executable's directory).
///
...
///
/// Where the staged dir lands: Windows bundles it beside `vozora.exe` (DLLs resolve
...
fn build_apple_intelligence_bridge() {
    ...
    // VOZORA_FORCE_AI_STUB=1 is an explicit escape hatch: force the stub even when
    ...
    let force_stub = env::var("VOZORA_FORCE_AI_STUB").as_deref() == Ok("1");
    ...
    if command_line_tools_only && !force_stub {
        println!(
            "cargo:warning=Command Line Tools-only toolchain detected; Apple Intelligence \
             ...
             `sudo xcode-select -s /Applications/Xcode.app`, or set VOZORA_FORCE_AI_STUB=1 to \
    ...
    let source_file = if has_foundation_models {
        ...
        // stubs — CLT-only detection and VOZORA_FORCE_AI_STUB (each warned about
