...
impl Default for KeyboardImplementation {
    fn default() -> Self {
        ...
        return KeyboardImplementation::VozoraKeys;
    }
}
...
/// How Vozora treats a transcription before it's pasted.
///
/// - `Literal`: paste exactly what was transcribed.
/// - `NaturalCorrected`: run the existing LLM post-processing pipeline
///   (grammar/filler-word cleanup) but nothing coding-specific.
/// - `AiPrompt`: treat the transcription as a prompt to send verbatim
///   (no coding-mode phrase substitution, no destructive-command gate) —
///   intended for dictating into a chat/prompt box such as Claude Code's.
/// - `Code`: run the coding-mode phrase table (see `coding_mode.rs`) so
///   spoken punctuation ("open paren", "abre llave", ...) becomes literal
///   characters, for dictating directly into an editor like Cursor.
/// - `TerminalCommand`: run coding-mode AND, if the resulting text matches
///   `coding_mode::looks_destructive`, require an explicit user confirmation
///   before the text is pasted (see `PendingPaste` in `actions.rs`). Vozora
///   never executes shell commands itself — this only gates *inserting the
///   text*, as a misheard-transcription safety net for terminals.
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq, Type, Default)]
#[serde(rename_all = "snake_case")]
...
/// Per-application dictation defaults, keyed by a case-insensitive substring
/// match against the focused window's title (see `app_profile.rs`). Lets a
/// user say "when Cursor is focused, always use Code mode" or "when a
/// terminal is focused, use TerminalCommand mode with the Direct paste
/// method", without changing their global default.
#[derive(Serialize, Deserialize, Debug, Clone, Type)]
pub struct AppProfile {
    /// Human-readable label shown in settings, e.g. "Cursor".
    pub name: String,
    /// Case-insensitive substring(s) matched against the OS-reported focused
    /// window title. Matches if ANY pattern is a substring of the title.
...
fn default_app_profiles() -> Vec<AppProfile> {
    vec![
        ...
        AppProfile {
            name: "Claude Code (terminal)".to_string(),
            window_title_patterns: vec!["claude".to_string()],
            dictation_mode: DictationMode::AiPrompt,
            paste_method: None,
        },
    ]
}
...
/// and `Dark` force one of the two palettes Vozora already ships.
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq, Type)]
#[serde(rename_all = "snake_case")]
pub enum Theme {
    System,
...
/* still vozora for composing the initial JSON in the store ------------- */
...
pub struct AppSettings {
    ...
    /// Global default dictation mode, used when no `app_profiles` entry
    /// matches the currently focused window.
    #[serde(default)]
    pub dictation_mode: DictationMode,
    /// Per-application dictation-mode / paste-method overrides. See
    /// `AppProfile` and `app_profile::resolve_dictation_mode`.
...
fn default_paste_delay_after_ms() -> u64 {
    // 60ms was too tight: on Windows, paste_via_clipboard (clipboard.rs)
    // restores the pre-dictation clipboard contents this many ms after
    // sending Ctrl+V. Lightweight native apps read the clipboard almost
    // immediately, but heavier ones (browser tabs, Electron apps) can take
    // longer than 60ms to actually process the paste — Vozora would then
    // silently wipe the clipboard back to its old contents before the
    // target app ever read the dictated text, so nothing appeared to be
    // pasted at all. 300ms gives real headroom without being noticeable.
    300
}
...
impl AppSettings {
    /// Debug-printable clone with secrets masked. API keys must never land in
    /// vozora.log — the file outlives the session and is the first thing users
...
/// In-memory copy of the last loaded/written settings. `get_settings` is on
/// hot paths (every shortcut keypress, the 10s idle watcher, most commands),
/// and re-running the store read + serde deserialization + migration checks on
/// each call is wasted work: every mutation in this module funnels through
/// `write_settings` or the load path below, so the cache can only go stale if
/// the store file is edited externally while the app runs (restart to pick
/// that up — same behavior as before, just formalized).
static SETTINGS_CACHE: RwLock<Option<AppSettings>> = RwLock::new(None);

pub fn get_settings(app: &AppHandle) -> AppSettings {
    if let Some(cached) = SETTINGS_CACHE
        .read()
    ...
    *SETTINGS_CACHE.write().unwrap_or_else(|e| e.into_inner()) = Some(settings.clone());
    settings
}

/// Uncached read: store fetch + deserialization + one-time migrations. Only
/// `get_settings` (on cache miss) should call this.
...
mod tests {
    ...
    fn frozen_v0_9_store_parses_strictly_without_migration() {
        ...
        let stored: serde_json::Value = serde_json::from_str(
            ...
            "experimental_enabled": false,
            "lazy_stream_close": false,
            "keyboard_implementation": "vozora_keys",
            "show_tray_icon": true,
            "paste_delay_ms": 60,
            "typing_tool": "auto",
    ...
    fn salvage_drops_only_wrong_typed_fields() {
        ...
        map.insert("custom_words".into(), serde_json::json!(["vozora"]));
        ...
        assert_eq!(salvaged.sound_theme, default_sound_theme());
        assert_eq!(salvaged.custom_words, vec!["vozora".to_string()]);
    }
