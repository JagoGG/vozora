...
impl TranscriptionManager {
    ...
    pub fn load_model_with_device(
        ...
        let emit_loading_failed = |error_msg: &str| {
            // This used to be silent on the backend side — the frontend showed
            // "Failed" but nothing ever landed in the log file, making every
            // real load failure undiagnosable after the fact. Always log it.
            log::error!("Model load failed for {}: {}", model_id, error_msg);
            let _ = self.app_handle.emit(
                "model-state-changed",
                ModelStateEvent {
                    event_type: "loading_failed".to_string(),
    ...
    pub fn initiate_model_load(&self) {
        // Claim the loading slot via the RAII guard so the flag is cleared and
        // waiters are woken even if the load thread panics. Clearing the flag
        // manually (the previous approach) left `is_loading` stuck true after a
        // panic, deadlocking every condvar waiter (start_stream, transcription)
        // and failing every subsequent model switch.
        ...
        if !reload_pending && self.is_model_loaded() {
            return; // guard drops here and releases the loading slot
        }
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        if std::env::var("VOZORA_FORCE_TRANSCRIPTION_FAILURE").is_ok() {
            return Err(anyhow::anyhow!(
                "Simulated transcription failure (VOZORA_FORCE_TRANSCRIPTION_FAILURE)"
            ));
        }
        ...
        let result = {
            ...
            if let LoadedEngine::TranscribeCpp(session) = &engine {
                let model = session.model();
                let caps = model.capabilities();
                // Informational only: the whisper run extension and fuzzy-
                // correction skip are gated on the actual architecture below.
...
/// Map Vozora's whisper accelerator setting to a transcribe-cpp [`Backend`].
///
