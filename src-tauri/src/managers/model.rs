...
/// Where a model comes from and how Vozora obtains it — the routing discriminant
...
/// The base code Vozora matches a language *intent* on: a tag's primary subtag,
...
/// Resolve a model file that's already been mirrored to the local, always-
/// on-disk models_dir (see `ModelManager::get_model_path`'s HuggingFace
/// branch). Checked *before* `hf_cached_path` wherever "is this model usable
/// right now" is decided: on this deployment the shared HF cache
/// (HF_HOME) was populated by a path that doesn't leave a proper blobs/
/// symlink behind (blobs/ holds only a stale .lock file even though the
/// snapshot file itself is a complete real file on disk), so hf-hub's own
/// `Cache::get()` intermittently reports the model as absent even when a
/// perfectly good local copy exists. A plain existence+size sanity check
/// against our own mirror is simple and doesn't depend on that crate's
/// internal bookkeeping.
fn local_mirror_path(models_dir: &Path, filename: &str) -> Option<PathBuf> {
...
/// Bridges hf-hub's async download progress to Vozora's `model-download-progress`
...
impl ModelManager {
    ...
    /// models dropped in or downloaded outside Vozora show up without a restart.
    ...
    fn update_download_status(&self) -> Result<()> {
        ...
        for model in models.values_mut() {
            if let ModelSource::HuggingFace { repo_id, revision } = &model.source {
                model.is_downloaded = local_mirror_path(&self.models_dir, &model.filename)
                    .is_some()
    ...
    fn discover_custom_transcribe_models(
        ...
        for entry in fs::read_dir(models_dir)? {
            ...
            // Vozora-downloaded one. Legacy `.bin` files have no GGUF header, so
    ...
    /// shared Hugging Face cache, so models downloaded by Vozora (or any other
    ...
    async fn download_hf_model(
        ...
        // Already mirrored locally, or in the shared cache (possibly from
        // another tool)? Done.
        if local_mirror_path(&self.models_dir, &filename).is_some()
    ...
    pub fn delete_model(&self, model_id: &str) -> Result<()> {
        ...
        if let ModelSource::HuggingFace { repo_id, revision } = &model_info.source {
            ...
            // Also remove our own local mirror (get_model_path), or it's left
            // as an orphaned multi-hundred-MB file the user can never see or
            // reclaim through the UI.
            if let Some(mirror) = local_mirror_path(&self.models_dir, &model_info.filename) {
                info!("Deleting local model mirror at: {:?}", mirror);
    ...
    pub fn get_model_path(&self, model_id: &str) -> Result<PathBuf> {
        ...
        if let ModelSource::HuggingFace { repo_id, revision } = &model_info.source {
            // Check our own local mirror first (see local_mirror_path's doc
            // comment: this deployment's shared HF cache doesn't leave a
            // proper blobs/ symlink behind, so hf-hub's Cache::get() can
            // report the model missing even when a complete local copy sits
            // right there — never let that flakiness block a load when we
            // already have a good local file).
            let local_mirror = self.models_dir.join(&model_info.filename);
            if local_mirror_path(&self.models_dir, &model_info.filename).is_some() {
                return Ok(local_mirror);
            }

            let cached_path = hf_cached_path(repo_id, revision, &model_info.filename)
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "Complete model file not found locally or in HF cache: {}",
                        model_id
                    )
                })?;

            // The shared HF cache (HF_HOME) can live on a network drive shared
            // across tools/machines (it does on this deployment: HF_HOME points
            // at a network share). Reading a multi-hundred-MB model straight
            // off the network on every load makes loads slow and, worse,
            // occasionally fails outright on transient network hiccups mid-read
            // (root cause of intermittent "loading_failed" model-load errors
            // that never showed up in logs because the failure path only
            // emitted a frontend event, never a log line — see emit_loading_failed
            // below). Mirror the file to the local, always-on-disk models_dir
            // once; every load after that reads a local file, immune to
            // network flakiness.
            {
                info!(
                    "Mirroring HF-cached model {} to local disk ({} -> {})",
                    model_id,
                    cached_path.display(),
                ...
                let tmp_path = self
                    .models_dir
                    .join(format!("{}.mirror-tmp", &model_info.filename));
                fs::copy(&cached_path, &tmp_path).map_err(|e| {
                    anyhow::anyhow!(
                        "Failed to mirror model {} from network cache to local disk: {}",
                        model_id,
                        e
                    )
                })?;
                fs::rename(&tmp_path, &local_mirror).map_err(|e| {
                    anyhow::anyhow!(
                        "Failed to finalize local mirror for model {}: {}",
                        model_id,
                        e
                    )
                })?;
                info!("Local mirror ready for model {}", model_id);
            }
            return Ok(local_mirror);
        }
