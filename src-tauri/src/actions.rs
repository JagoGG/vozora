        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Some(samples) = rm.stop_recording(&binding_id, cancel_generation) {
                ...
                if samples.is_empty() {
                    ...
                    let file_name = format!("vozora-{}.wav", chrono::Utc::now().timestamp());
                    ...
                    match transcription_result {
                        Ok(transcription) => {
                            ...
                            if processed.final_text.is_empty() {
                                ...
                                if matches!(
                                    dictation_mode,
                                    crate::settings::DictationMode::Code
                                        | crate::settings::DictationMode::TerminalCommand
                                ) {
                                    final_text = crate::coding_mode::apply_coding_mode(&final_text);
                                }
                                ...
                                if is_destructive_terminal_command {
                                    ...
                                    // command in terminal-command mode. Stash it and ask the
                                    // frontend to confirm; Vozora still never executes anything
                                    // itself, this only gates the *paste*.
                                    warn!(
                                        "Withholding auto-paste: transcription looks destructive in TerminalCommand mode"
                                    );
                                    if let Some(pending_state) =
                                        ah.try_state::<crate::commands::dictation::PendingPasteState>()
                                    {
                                        if let Ok(mut guard) = pending_state.0.lock() {
                                    ...
                                    ah.run_on_main_thread(move || {
                                        ...
                                        match utils::paste_with_method_override(
                                            final_text,
                                            ah_clone.clone(),
                                            paste_method_override,
                                        ) {
                        ...
                        Err(err) => {
                            ...
                            // message is also in vozora.log via the line above.
