...
//! - `vozora_keys`: Uses the vozora-keys library for more control
//!
...
// Note: Commands are accessed via shortcut::vozora_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::VozoraKeys => {
            if let Err(e) = vozora_keys::init_shortcuts(app) {
                error!("Failed to initialize vozora-keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry VozoraKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::VozoraKeys => vozora_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::VozoraKeys => vozora_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::VozoraKeys => vozora_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::VozoraKeys => vozora_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::VozoraKeys => "vozora_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::VozoraKeys => vozora_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "vozora_keys" => KeyboardImplementation::VozoraKeys,
        other => {
            warn!(
...
/// Initialize VozoraKeys if not already initialized, with rollback on failure
fn initialize_vozora_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<vozora_keys::VozoraKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = vozora_keys::init_shortcuts(app) {
        error!("Failed to initialize VozoraKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize VozoraKeys: {}. Reverted to Tauri.",
            e
        ));
    }
