...
impl VozoraKeysState {
    /// Create a new VozoraKeysState
    ...
    fn recording_loop(app: AppHandle, running: Arc<AtomicBool>) {
        while running.load(Ordering::SeqCst) {
            let event = {
                let state = match app.try_state::<VozoraKeysState>() {
                    Some(s) => s,
                    None => break,
                };
            ...
            if let Some(key_event) = event {
                ...
                // Emit to frontend
                if let Err(e) = app.emit("vozora-keys-event", &frontend_event) {
...
/// Validate a shortcut string for the VozoraKeys implementation.
/// VozoraKeys is more permissive: allows modifier-only combos and the fn key.
pub fn validate_shortcut(raw: &str) -> Result<(), String> {
    if raw.trim().is_empty() {
        return Err("Shortcut cannot be empty".into());
    }
    // VozoraKeys accepts modifier-only, key-only, and modifier+key combos
    // Just verify the string is parseable
    raw.parse::<Hotkey>()
        .map(|_| ())
        .map_err(|e| format!("Invalid shortcut for VozoraKeys: {}", e))
}

/// Initialize handy-keys shortcuts
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    let state = VozoraKeysState::new(app.clone())?;
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Some(cancel_binding) = get_settings(&app_clone).bindings.get("cancel").cloned() {
                if let Some(state) = app_clone.try_state::<VozoraKeysState>() {
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    let state = app
        .try_state::<VozoraKeysState>()
        .ok_or("VozoraKeysState not initialized")?;
...
#[tauri::command]
#[specta::specta]
pub fn start_vozora_keys_recording(app: AppHandle, binding_id: String) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::VozoraKeys {
...
#[tauri::command]
#[specta::specta]
pub fn stop_vozora_keys_recording(app: AppHandle) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::VozoraKeys {
        return Err("handy-keys is not the active keyboard implementation".into());
    }
